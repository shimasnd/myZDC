...
namespace RawTowerDefs
{
  ...
  {
//     static unsigned int bitsIndex1 = 9;   
    static unsigned int bitsIndex2 = 9; // max 0x1FF (511)
    static unsigned int bitsIndex3 = 6;  // max 0x3F (63)

    return (calo_tower_id >> (bitsIndex2+bitsIndex3)) & 0x1FF;
  }
  ...
  decode_index2zdc(const unsigned int calo_tower_id)
  {
    ...
    return (calo_tower_id >> (bitsIndex3)) & 0x1FF;
  }
  ...
  {
    unsigned int calo_id = decode_caloid(calo_tower_id);
    if(calo_id==RawTowerDefs::ZDC_Crystal || 
       calo_id==RawTowerDefs::ZDC_SiPixel ||
       calo_id==RawTowerDefs::ZDC_SiPad  ||
       calo_id==RawTowerDefs::ZDC_Sci)
    {
      return decode_index1zdc(calo_tower_id);
    }
  ...
  decode_index2(const unsigned int calo_tower_id)
  {
    ...
    if(calo_id==RawTowerDefs::ZDC_Crystal || 
       ...
       calo_id==RawTowerDefs::ZDC_Sci)
    {
      return decode_index2zdc(calo_tower_id);
    }
...
//     decode_index3v2(calo_tower_id);
   ...
   */
  inline RawTowerDefs::keytype
  encode_towerid_zdc(const CalorimeterId calo_id, const unsigned int tower_index_1,
		     const unsigned int tower_index_2, const unsigned int tower_index_3)
  {
    RawTowerDefs::keytype calo_tower_id = 0;

//     static unsigned int bitsIndex1 = 9; // max 0x1FF (511)
    static unsigned int bitsIndex2 = 9; // max 0x1FF (511)
    static unsigned int bitsIndex3 = 6;  // max 0x3F (63)
    
    if (tower_index_1 < 0x1FF && tower_index_2 < 0x1FF &&  tower_index_3 < 0x3F)
    {
      calo_tower_id = (calo_id << RawTowerDefs::tower_idbits) + (tower_index_1 << (bitsIndex2+bitsIndex3)) + (tower_index_2 << bitsIndex3) + tower_index_3;
    }
    else
    {
      std::cout << "too large index1 and/or index2; index1: "
                << tower_index_1 << " (max val " << 0x1FF << ")"
                << ", index2: "
                << tower_index_2 << " (max val " << 0x1FF << ")"
                << ", index3: "
                << tower_index_3 << " (max val " << 0x3F << ")"<< std::endl;
      exit(1);
    }
